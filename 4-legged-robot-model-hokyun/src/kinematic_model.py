...
class robotKinematics:
    ...
    def solve(self, orn , pos , bodytoFeet):
        # robot_walking.py에서 gaitplanner.py을 통해 보행 궤적을 구해 kinematic_model.py를 실행함.
        # 발 프레임 벡터 저장
        ...
        # 4개의 정점 벡터(바디 -> 고관절 벡터, body->coxa)를
        # geometrics.py를 이용해 회전변환, 좌표변환을 통해 계산 (현재 바디프레임-고관절 프레임 벡터, 오리엔테이션 벡터, 포지션 벡터 사용)
        ...
        # IK를 풀기 위해 필요한 고관절-발 벡터를 구하기 위한 과정
        # (고관절->발 벡터) = (바디 -> 발 벡터) - (바디 -> 고관절 벡터)
        ...
        # (고관절->발 벡터)의 변환을 undo 하는 과정을 거침, 다리 벡터를 따로 저장해놓기 위한 과정이라고 함
